<div class=" card my-2 p-5 sm:p-10  md:w-[48%] lg:w-[32%]">
    ...
    <p>{{ __('txt.service_card.intervention_domains') }} <span class="font-bold">{{ __('txt.intervention_domains.education') }} </span> </p>
    ...
    {{ __('txt.service_card.online') }} </p>
        <p class="flex">               
    <x-heroicon-o-check-circle class="h-5 w-5 text-gray1 fill-green-300 mr-1" />
    {{ __('txt.service_card.email') }}</p>
        <p class="flex">               
    <x-heroicon-o-check-circle class="h-5 w-5 text-gray1 fill-green-300 mr-1" />
    {{ __('txt.service_card.location') }} </p>
    </div>
    <button
...
<div class=" card my-2 p-5 sm:p-10  md:w-[48%] lg:w-[32%]">
    ...
    <p class="my-2 flex">               
        <x-heroicon-o-location-marker class="h-5 w-5 text-gray1 mr-3" />
        {{ __('txt.service_card.city_any') }}
        </p>   
    <h4 class="my-2 text-xl font-bold">Adapost persoane</h4>
    <p>{{ __('txt.service_card.project_name') }} </p>
    <p>{{ __('txt.service_card.provided_by') }} <span class="font-bold">ONG X</span> </p>
    <p>{{ __('txt.service_card.intervention_domains') }} <span class="font-bold">{{ __('txt.intervention_domains.protection') }} </span> </p>
    ...
    <p> <span class="font-bold">{{ __('txt.service_card.service_access') }}  </span> </p>
    <p class="flex">               
        <x-heroicon-o-x-circle class="h-5 w-5 text-gray1 mr-1" />{{ __('txt.service_card.project_finished') }} </p>
    <button
                        class="my-2 mt-3 h-12 w-full bg-orange1  text-black rounded-md hover:bg-blue1 md:mt-auto">{{ __('txt.buttons.see_more') }}</button>
</div>